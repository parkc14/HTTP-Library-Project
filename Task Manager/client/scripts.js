...
const update = document.querySelector("#updateitem");
const updateInput = document.getElementById("updateInput");
const addButton = document.querySelector(".add-btn");
const delButton = document.querySelector(".del-btn");
const updButton = document.querySelector(".upd-btn");
const compButton = document.querySelector(".comp-btn");
const confButton = document.querySelector(".conf-btn");
const label = document.querySelector(".listLabel");

// Add event listeners to the buttons to handle click events
addButton.addEventListener("click", httpPost);
delButton.addEventListener("click", httpDelete);
updButton.addEventListener("click", httpPatch);
compButton.addEventListener("click", httpCompleted);
confButton.addEventListener("click", httpConfirm);
...
async function httpConfirm(e) {
  const oldItem = input.value.trim();
  const itemUpdate = update.value.trim();
  ...
  if (oldItem === "") {
    ...
    const index = theList.indexOf(oldItem);
    
    if (index >= 0) {
      
      theList[index] = itemUpdate;
      ShowList();
      await WriteList();
    }
    else {
      alert(`Item "${oldItem}" not found in the list.`);
      return;
    }
  }
  input.value = "";
...
async function httpCompleted(e){
  e.preventDefault();
  const itemCompleted = input.value.trim();
  const index = theList.indexOf(itemCompleted);
  
    if (index > -1 || itemCompleted != "") {
      theList[index] = theList[index].strike();
      ShowList();
      WriteList();
      input.value = "";
    } 
...
async function httpDelete(e) {
  ...
  // if (theList[index].includes('<strike>')) {
  //   // Remove the <strike> tags
  //   theList[index] = theList[index].replace(/<\/?strike>/g, '');
  // }
  if (itemToDelete === "") {
